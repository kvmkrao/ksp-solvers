...
int main(int argc,char **args)
{
  ...
  PetscErrorCode ierr;
  PetscInt       i,M = 288420,N,col[3],its,rstart,rend,nlocal;
  PetscInt       start, end,rank,size,irow,icol;
  PetscInt       istart, iend,m,n,pst,ped,lor;
...
//  ierr = PetscInitialize(NULL, NULL, (char*)0, help); CHKERRQ(ierr);
  ...
  ierr = MPI_Comm_size(PETSC_COMM_WORLD,&size);CHKERRQ(ierr);
   ...
   // M - global number of rows/equations 
   // m - number of rows on a processor 
   // N - global number of columns 
   // n - number of columns on a processor
   m = M/size; 
   if(rank == size-1) m = M - (size-1)*m;  
   cout << "rows" << m <<" "<<rank << endl; 
   n = m;
   N = M;  
...
//  ierr = MatSetOption(A,MAT_SPD,PETSC_TRUE);CHKERRQ(ierr);
  ...
  pst = rank*m ;
  ped = (rank+1)*m; 
  // more information about o_nnz and d_nnz can be found at 
 ...
 /* 
  cout << "allocate d_nnz, o_nnz rrays" << endl;
  for (i=0; i<max; i++) { 
      matfile >> irow >> icol >> val1;
      if(irow >= pst && irow < ped) { //check the process own the row or not 
//         local row number - lor 
           lor  = irow - (rank)*m; 
           if(icol >= pst && icol < ped) { 
		   lor  = irow - (rank+1)*m;  
		   d_nnz[lor] = d_nnz[lor]+1; 
           }
	   else  {
		   o_nnz[lor] = o_nnz[lor]+1;
	   }
	  //if(rank==0) cout << " myrank" << rank << " "<< irow << " " << m <<" "<<lor << endl; 
      }
  }
  matfile.close();
  */
  ...
  ierr = MatMPIAIJSetPreallocation(A,463,NULL,463,NULL);CHKERRQ(ierr);
  ierr = MatSeqAIJSetPreallocation(A,463,NULL);CHKERRQ(ierr);
//  ierr = MatSeqAIJSetPreallocation(A,0,d_nnz);CHKERRQ(ierr);  //ex56.c
//  ierr = MatMPIAIJSetPreallocation(A,NULL,d_nnz,0,o_nnz);CHKERRQ(ierr);
...
//  PetscPrintf(PETSC_COMM_WORLD,"%d %d %d \n",rank, start, end);
  ...
  ifstream mfile; 
  mfile.open("sparsemat_S0.dat");
  if(!mfile) {
             cout << "Error in opening the file" << std::endl;
  }
  mfile >> word >> word >> word ;

  for (i=0; i<max; i++) { 
      mfile >> irow >> icol >> val1;
