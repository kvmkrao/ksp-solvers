...
int itersolve(int &nn, int &ent, int *row, int *col1,double *rhs,double *xv, double *val)
{
/*
  cout << nn << std:: endl; 
  for (int i=0;  i < 100; i++) {
...
//  PetscPrintf(PETSC_COMM_WORLD,"%d %d %d \n",rank, start, end);
  ...
  for (i=0; i<ent; i++) { 
      ...
      val1 = val[i]; 
      if(ii <n && jj < n) {
        MatSetValues(A,1,&ii,1,&jj,&val1,INSERT_VALUES);
      }
  }
...
//  VecDuplicate(x,&u);
  ...
  for (i=0; i<n; i++) {
     ...
     ierr = VecSetValues(x,1,&i,&xv[i], INSERT_VALUES);CHKERRQ(ierr);
  }
