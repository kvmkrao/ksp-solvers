...
//  PetscPrintf(PETSC_COMM_WORLD,"%d %d %d \n",rank, start, end);
  ...
  for (i=0; i<max; i++) { 
      ...
      if(irow <n && icol < n) {
        MatSetValues(A,1,&irow,1,&icol,&val1,INSERT_VALUES);
      }
  }
  matfile.close();
...
*/
  ...
  for (i=0; i<n; i++) {
     rfile >> ii >> rhs1 ;
     xfile >> val1; 
     ierr = VecSetValues(b,1,&i,&rhs1,INSERT_VALUES);CHKERRQ(ierr);
     ierr = VecSetValues(x,1,&i,&val1,INSERT_VALUES);CHKERRQ(ierr);
  }
  rfile.close(); 
  xfile.close(); 
...
*/
  ...
  // print timings 
  std::cout << "setup time "          << (c_start1 - c_start0)/CLOCKS_PER_SEC << std::endl; 
  std::cout << "fill matrix: time "   << (c_start2 - c_start1)/CLOCKS_PER_SEC << std::endl; 
  std::cout << "set up vectors: time "<< (c_start3 - c_start2)/CLOCKS_PER_SEC << std::endl; 
  std::cout << "linear solve time "   << (c_start4 - c_start3)/CLOCKS_PER_SEC << std::endl; 
  std::cout << "Total "               << (c_start4 - c_start0)/CLOCKS_PER_SEC << std::endl; 
   /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
